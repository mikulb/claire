...
class results_analysis_beta(models.Model):
	_name = 'results.analysis.beta'
	...
	number_line = fields.One2many('number.line.analysis', 'rab_id' )
	consecutive_line = fields.One2many('consecutive.line.analysis', 'rab_id' )
	highlow_line = fields.One2many('highlow.line.analysis', 'rab_id' )
	oddeven_line = fields.One2many('oddeven.line.analysis', 'rab_id' )
	...
	def generate_dated_analysis(self):
		...
		number_line_obj = self.pool.get('number.line.analysis')
		consecutive_line_obj = self.pool.get('consecutive.line.analysis')
		highlow_line_obj = self.pool.get('highlow.line.analysis')
		oddeven_line_obj = self.pool.get('oddeven.line.analysis')
		...
		for row in result:
			number_line_obj.create(cr, uid, {'sort':1, 'name':'zero','analysis_by_date':row['zero'],'rab_id':self.id}, context=context)
			self.zero = row['zero']
			self.one = row['one']
		...
		query="""
				SELECT
					lr.date::DATE,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 6 AND even = 0)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as zero_six,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 5 AND even = 1)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as one_five,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 4 AND even = 2)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as two_four,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 3 AND even = 3)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as three_three,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 2 AND even = 4)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as four_two,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 1 AND even = 5)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as five_one,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND odd = 0 AND even = 6)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
					), 2) *100 as six_zero,
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE ) as all
		...
		for row in result:
			#print row
			self.odd_even_06 = row['zero_six']
			self.odd_even_15 = row['one_five']
			self.odd_even_24 = row['two_four']
			self.odd_even_33 = row['three_three']
			self.odd_even_42 = row['four_two']
			self.odd_even_51 = row['five_one']
			self.odd_even_60 = row['six_zero']

		query="""
				SELECT
				lr.date::DATE,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 6 AND low = 0)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as zero_six,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 5 AND low = 1)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as one_five,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 4 AND low = 2)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as two_four,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 3 AND low = 3)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as three_three,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 2 AND low = 4)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as four_two,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 1 AND low = 5)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as five_one,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE AND high = 0 AND low = 6)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE )::DECIMAL
				), 2) *100 as six_zero,
				(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND date_drawn <= lr.date::DATE ) as all
		...
		for row in result:
			#print row
			self.high_low_06 = row['zero_six']
			self.high_low_15 = row['one_five']
			self.high_low_24 = row['two_four']
			self.high_low_33 = row['three_three']
			self.high_low_42 = row['four_two']
			self.high_low_51 = row['five_one']
			self.high_low_60 = row['six_zero']
		...
		query="""
				...
				lr.date::DATE,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 6 AND even = 0)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as zero_six,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 5 AND even = 1)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as one_five,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 4 AND even = 2)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as two_four,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 3 AND even = 3)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as three_three,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 2 AND even = 4)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as four_two,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 1 AND even = 5)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as five_one,
					round((
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND odd = 0 AND even = 6)::DECIMAL
						/
						(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
					), 2) *100 as six_zero,
		...
		for row in result:
			#print row
			self.b_odd_even_06 = row['zero_six']
			self.b_odd_even_15 = row['one_five']
			self.b_odd_even_24 = row['two_four']
			self.b_odd_even_33 = row['three_three']
			self.b_odd_even_42 = row['four_two']
			self.b_odd_even_51 = row['five_one']
			self.b_odd_even_60 = row['six_zero']

		query="""
				SELECT
				lr.date::DATE,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 6 AND low = 0)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as zero_six,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 5 AND low = 1)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as one_five,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 4 AND low = 2)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as two_four,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 3 AND low = 3)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as three_three,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 2 AND low = 4)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as four_two,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 1 AND low = 5)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as five_one,
				round((
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) AND high = 0 AND low = 6)::DECIMAL
					/
					(SELECT count(*) FROM lotto_records WHERE year::INT = EXTRACT( year from lr.date::DATE) )::DECIMAL
				), 2) *100 as six_zero,
		...
		for row in result:
			#print row
			self.b_high_low_06 = row['zero_six']
			self.b_high_low_15 = row['one_five']
			self.b_high_low_24 = row['two_four']
			self.b_high_low_33 = row['three_three']
			self.b_high_low_42 = row['four_two']
			self.b_high_low_51 = row['five_one']
			self.b_high_low_60 = row['six_zero']


class number_line_analysis(models.Model): 
	_name = 'number.line.analysis'
	_order = 'sort'

	sort = fields.Integer(string="Sort")
	name = fields.Char(string="Number")
	analysis_by_date = fields.Float(string="Analysis by Date")
	prediction = fields.Float(string="Prediction")
	statistics_year = fields.Float(string="Stats (Year)")
	statistics_all = fields.Float(string="Stats (All)")

	rab_id = fields.Many2one('results.analysis.beta', 'number_line', store = True,ondelete='cascade')

class consecutive_line_analysis(models.Model): 
	_name = 'consecutive.line.analysis'
	_order = 'sort'

	sort = fields.Integer(string="Sort")
	name = fields.Char(string="Consecutive")
	analysis_by_date = fields.Float(string="Analysis by Date")
	prediction = fields.Float(string="Prediction")
	statistics_year = fields.Float(string="Stats (Year)")
	statistics_all = fields.Float(string="Stats (All)")

	rab_id = fields.Many2one('results.analysis.beta', 'consecutive_line', store = True,ondelete='cascade')

class highlow_line_analysis(models.Model): 
	_name = 'highlow.line.analysis'
	_order = 'sort'

	sort = fields.Integer(string="Sort")
	name = fields.Char(string="High-Low")
	analysis_by_date = fields.Float(string="Analysis by Date")
	prediction = fields.Float(string="Prediction")
	statistics_year = fields.Float(string="Stats (Year)")
	statistics_all = fields.Float(string="Stats (All)")

	rab_id = fields.Many2one('results.analysis.beta', 'highlow_line', store = True,ondelete='cascade')

class oddeven_line_analysis(models.Model): 
	_name = 'oddeven.line.analysis'
	_order = 'sort'

	sort = fields.Integer(string="Sort")
	name = fields.Char(string="Odd-Even")
	analysis_by_date = fields.Float(string="Analysis by Date")
	prediction = fields.Float(string="Prediction")
	statistics_year = fields.Float(string="Stats (Year)")
	statistics_all = fields.Float(string="Stats (All)")

	rab_id = fields.Many2one('results.analysis.beta', 'oddeven_line', store = True,ondelete='cascade')
...
class results_analysis(models.Model):
	...
	high_low_06 = fields.Float(string="0 - 6")
	high_low_15 = fields.Float(string="1 - 5")
	high_low_24 = fields.Float(string="2 - 4")
	high_low_33 = fields.Float(string="3 - 3")
	high_low_42 = fields.Float(string="4 - 2")
	high_low_51 = fields.Float(string="5 - 1")	
	high_low_60 = fields.Float(string="6 - 0")		

	a_high_low_06 = fields.Float(string="0 - 6")
	a_high_low_15 = fields.Float(string="1 - 5")
	a_high_low_24 = fields.Float(string="2 - 4")
	a_high_low_33 = fields.Float(string="3 - 3")
	a_high_low_42 = fields.Float(string="4 - 2")
	a_high_low_51 = fields.Float(string="5 - 1")
	a_high_low_60 = fields.Float(string="6 - 0")

	b_high_low_06 = fields.Float(string="0 - 6")
	b_high_low_15 = fields.Float(string="1 - 5")
	b_high_low_24 = fields.Float(string="2 - 4")
	b_high_low_33 = fields.Float(string="3 - 3")
	b_high_low_42 = fields.Float(string="4 - 2")
	b_high_low_51 = fields.Float(string="5 - 1")
	b_high_low_60 = fields.Float(string="6 - 0")


	odd_even_06 = fields.Float(string="0 - 6")
	odd_even_15 = fields.Float(string="1 - 5")
	odd_even_24 = fields.Float(string="2 - 4")
	odd_even_33 = fields.Float(string="3 - 3")
	odd_even_42 = fields.Float(string="4 - 2")
	odd_even_51 = fields.Float(string="5 - 1")
	odd_even_60 = fields.Float(string="6 - 0")

	a_odd_even_06 = fields.Float(string="0 - 6")
	a_odd_even_15 = fields.Float(string="1 - 5")
	a_odd_even_24 = fields.Float(string="2 - 4")	
	a_odd_even_33 = fields.Float(string="3 - 3")
	a_odd_even_42 = fields.Float(string="4 - 2")
	a_odd_even_51 = fields.Float(string="5 - 1")
	a_odd_even_60 = fields.Float(string="6 - 0")

	b_odd_even_06 = fields.Float(string="0 - 6")
	b_odd_even_15 = fields.Float(string="1 - 5")
	b_odd_even_24 = fields.Float(string="2 - 4")
	b_odd_even_33 = fields.Float(string="3 - 3")
	b_odd_even_42 = fields.Float(string="4 - 2")
	b_odd_even_51 = fields.Float(string="5 - 1")
	b_odd_even_60 = fields.Float(string="6 - 0")
	...
	@api.multi
	def get_frequent_combination(self):
		print 'get_frequent_combination'
		...
		for row in alist:
			for row_b in alist:
				if row != row_b:
					if row_b > row:
						txt = "x%s - %sx"%(row,row_b)
						#comb.append(txt.replace('x','%'))
		#print comb
		...
		for row in comb:
			...
			query="""
					SELECT count(name) as count FROM lotto_records WHERE sorted_name like '%s'
			"""%(row)
			...
			print result
			print {'Comb':row.replace('%',''), 'Count':int(result[0])}
			#print x[count]
